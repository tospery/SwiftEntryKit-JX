...
final public class EKAlertMessageView: EKSimpleMessageView, EntryAppearanceDescriptor {
    ...
    var bottomCornerRadius: CGFloat = 0 {
        didSet {
            buttonBarView.bottomCornerRadius = buttonBarView.buttonBarContent.margin.vertical != 0 ? 0 : bottomCornerRadius
        }
    }
    ...
    func layoutContent(with message: EKAlertMessage) {
        ...
        buttonBarView.layoutToSuperview(axis: .horizontally, offset: buttonBarView.buttonBarContent.margin.horizontal)
        buttonBarView.layoutToSuperview(.bottom, offset: -buttonBarView.buttonBarContent.margin.vertical)
